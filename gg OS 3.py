...
print('app, app2,  cmd , browser, weather, bios, auction,  suprise, bomber, game, troled  q = выйти')
...
if c == 'help':
    ...
    print('ты за-тролен  ты затролен  этим  явно не доволен от ответа толку  нет он начнёт острить момент над тобой нет постой непеши он троль ждёт пока у тебя рванёт пока он бородку мнёт ты затролен ты затролен прекращай пока способен выиграть можно не начав ответ огня ты всё продолжаешь отвечать ты затролен ты затролен хорошего дня')
